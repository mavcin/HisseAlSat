...
namespace TS.EasyStockManager.Service.Transaction
{
    public class TransactionService : BaseService, ITransactionService
    {
        ...
        public async Task<ServiceResult> AddAsync(TransactionDTO model)
        {
            ...
            try
            {
                if (model.ToStoreId == model.TelegramId)
                {
                ...
                using (_unitOfWork)
                {
                    ...
                    foreach (var transactionDetailItem in model.TransactionDetail)
                    {
                        ...
                        Data.Entity.TelegramStock storeStock = await _unitOfWork.StoreStockRepository.GetByStoreAndProductId(transactionDetailItem.ProductId.Value, model.TelegramId.Value);

                        if (storeStock == null)
                        {
                            storeStockIsUpdate = false;
                            storeStock = new Entity.TelegramStock();
                            storeStock.TelegramId = model.TelegramId.Value;
                        ...
                        if (model.TransactionTypeId == (int)TransactionType.Transfer && model.ToStoreId.HasValue)
                        {
                            ...
                            Data.Entity.TelegramStock storeStockTransfer = await _unitOfWork.StoreStockRepository.GetByStoreAndProductId(transactionDetailItem.ProductId.Value, model.ToStoreId.Value);
                            if (storeStockTransfer == null)
                            {
                                storeStockTransferIsUpdate = false;
                                storeStockTransfer = new Entity.TelegramStock();
                                storeStockTransfer.TelegramId = model.ToStoreId.Value;
                ...
                using (_unitOfWork)
                {
                    IEnumerable<Entity.Transaction> list = await _unitOfWork
                                                                ...
                                                                .FindAsync(filter: x => (string.IsNullOrEmpty(criteria.TransactionCode) || x.TransactionCode.Contains(criteria.TransactionCode)) &&
                                                                                        ...
                                                                                        (criteria.TelegramId == null || x.StoreId == criteria.TelegramId) &&
            ...
            try
            {
                ...
                using (_unitOfWork)
                {
                    ...
                    if (entity != null)
                    {
                        ...
                        {
                            Data.Entity.TelegramStock storeStock = await _unitOfWork.StoreStockRepository.GetByStoreAndProductId(transactionDetailItem.ProductId, entity.StoreId);
                            if (storeStock != null)
                            {
                                if (model.TransactionTypeId == (int)TransactionType.StockOut || model.TransactionTypeId == (int)TransactionType.Transfer)
                        ...
                        if (model.TransactionTypeId == (int)TransactionType.Transfer)
                        {
                            foreach (var transactionDetailItem in entity.TransactionDetail)
                            {
                                Data.Entity.TelegramStock storeStock = await _unitOfWork.StoreStockRepository.GetByStoreAndProductId(transactionDetailItem.ProductId, entity.ToStoreId.Value);
                                if (storeStock != null)
                                    storeStock.Stock -= transactionDetailItem.Amount;
                            }
                        }
